"""
Tests for py_chainlink_streams.decode module.
"""
...
class TestGetSchemaVersion:
    ...
    def test_handles_uppercase_hex_characters(self):
        ...
        assert get_schema_version(feed_id) == 10
    ...
    def test_handles_mixed_case_hex_characters(self):
        ...
        assert get_schema_version(feed_id) == 15
...
class TestDecodeReportStructure:
    ...
    def test_returns_dict_with_correct_keys(self):
        ...
        result = decode_report_structure(hex_data)
...
class TestDecodeV3ReportData:
    ...
    def test_returns_dict_with_correct_v3_schema_fields(self):
        ...
        result = decode_v3_report_data(encoded)
...
class TestDecodeReport:
    ...
    def test_decodes_when_feed_id_provided(self):
        ...
        with pytest.raises((ValueError, Exception)):
            decode_report(hex_data, feed_id=feed_id)
    ...
    def test_decodes_when_schema_version_provided(self):
        ...
        with pytest.raises((ValueError, Exception)):
            decode_report(hex_data, schema_version=3)

    def test_raises_when_neither_feed_id_nor_schema_version_provided(self):
        """Test raises ValueError when neither feed_id nor schema_version provided."""
        with pytest.raises(ValueError, match="Either feed_id or schema_version must be provided"):
            decode_report("0x1234")
    ...
    def test_returns_dict_with_correct_keys(self):
        ...
        feed_id = "0x0003abc"
        with pytest.raises((ValueError, Exception)):
            result = decode_report("0x00", feed_id=feed_id)
...
class TestDecodeReportFromResponse:
    ...
    def test_raises_when_feed_id_missing(self):
        ...
        with pytest.raises(ValueError, match="must contain 'feedID' and 'fullReport' keys"):
            decode_report_from_response(response)
...
class TestConvertFixedPointToDecimal:
    ...
    def test_converts_with_default_18_decimals(self):
        ...
        result = convert_fixed_point_to_decimal(value)
        assert result == 1.0

    def test_converts_with_custom_decimals(self):
        """Test converts value with custom decimals."""
        value = 100000000  # 1 with 8 decimals
        result = convert_fixed_point_to_decimal(value, decimals=8)
        assert result == 1.0

    def test_returns_float(self):
        """Test returns float."""
        result = convert_fixed_point_to_decimal(1000000000000000000)
        assert isinstance(result, float)

    def test_handles_zero_value(self):
        """Test handles zero value."""
        result = convert_fixed_point_to_decimal(0)
        assert result == 0.0
    ...
    def test_works_with_different_decimal_places(self):
        ...
        value_8 = 100000000
        result_8 = convert_fixed_point_to_decimal(value_8, decimals=8)
        assert result_8 == 1.0
        
        # Test with 18 decimals (like Ethereum)
        value_18 = 1000000000000000000
        result_18 = convert_fixed_point_to_decimal(value_18, decimals=18)
        assert result_18 == 1.0


class TestGetDecodedPrices:
    """Test get_decoded_prices function."""
    ...
    def test_returns_dict_with_correct_keys(self, sample_decoded_report):
        ...
        result = get_decoded_prices(sample_decoded_report)
        assert 'observationsTimestamp' in result
        assert 'benchmarkPrice' in result
        assert 'bid' in result
        assert 'ask' in result
    ...
    def test_raises_for_non_v3_schema_reports(self):
        ...
        with pytest.raises(ValueError, match="only supports v3 schema reports"):
            get_decoded_prices(invalid_report)
    ...
    def test_uses_custom_decimals_when_provided(self, sample_decoded_report):
        ...
        result_18 = get_decoded_prices(sample_decoded_report, decimals=18)
        result_8 = get_decoded_prices(sample_decoded_report, decimals=8)
